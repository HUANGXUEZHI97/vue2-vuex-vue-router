<template>
  <section>
    <header class="top_tips">
      ...
      <span class="num_tip" v-else-if="fatherComponent == 'item'">题目{{itemNum}}</span>
    </header>
      ...
      <div class="item_back item_container_style">
          ...
          <header class="item_title">{{itemDetail[itemNum-1].topic_name}}</header>
					<ul>
						<li v-for="(item,index) in itemDetail[itemNum-1].topic_answer" @click="choosed(index,item.is_standard_answer)" class="item_list" :key="item.topic_answer_id">
...
export default {
  ...
  props:{
    ...
    choosed(type,id){
			this.choosedNum=type;//获得选中序号
			this.choosedId=id;//获得选中ID
		},
		//答完题目，提交答案
		submitAnswer(){
			if(this.choosedNum !== null){
